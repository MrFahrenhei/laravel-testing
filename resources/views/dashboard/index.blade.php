<x-layout>
    <section class="flex flex-coll md:flex-row gap-4">
...
{{--            php artisan storage:link--}}
        ...
        <div class="bg-white p-8 rounded-lg shadow-md w-full">
            ...
            @forelse($jobs as $job)
                <div class="flex justify-between items-center border-b-2 border-gray-200 py-2">
                    ...
                    <div class="flex space-x-3">
                        ...
                        <form method="POST" action="{{route('jobs.destroy', $job->id)}}?from=dashboard" onsubmit="return confirm('Are you sure?')">
                            @csrf
                            @method('DELETE')
                ...
                <div class="mt-4 bg-gray-100 p-2">
                    ...
                    @forelse($job->applicants as $applicant)
                        <div class="py-2">
                            ...
                            <form method="POST" action="{{route('applicant.destroy', $applicant->id)}}" onsubmit="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <p class="text-gray-700">You have not job</p>
            @endforelse
        </div>
    </section>
    <x-bottom-banner/>
